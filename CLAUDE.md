...
id8 (ideate) is an installer that provisions a multi-agent workflow (`/id8`) into target project directories. The workflow turns a prompt into a deployed Next.js app on Vercel through a 9-step pipeline. It uses MCP servers for documentation (Context7) and design (Stitch), and CLIs for GitHub, Vercel, and Supabase. Supports three AI agents: Claude Code, Codex, and Antigravity.
...
**Run non-interactive:**
```bash
python3 src/install_id8_workflow.py --non-interactive --project-dir ~/my-app --agents claude,codex
```
...
- **`Installer` class** — the core orchestrator. Resolves config (project dir, agents) via CLI args, env vars, or interactive prompts, then writes agent-specific assets into the target project.
...
1. `_load_mcp_servers()` reads `src/assets/id8/mcp_manifest.json` for server URLs and auth metadata (Context7 and Stitch only).
...
Only **Context7** (docs) and **Stitch** (design) use MCP servers. The workflow uses CLIs for everything else:
- **GitHub**: `gh` CLI for repo creation and code push
- **Vercel**: `vercel` CLI (via npx) for deployment
- **Supabase**: `supabase` CLI for backend/database setup (optional)

### Asset structure under `src/assets/id8/`

- `mcp_manifest.json` — single source of truth for MCP server URLs and auth keys (Context7 + Stitch)
...
- MCP server names in output configs are not prefixed (e.g., `context7`, not `id8_context7`).
